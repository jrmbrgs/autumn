...
SELENIUM_OPT="-role node -hub http://127.0.0.1:4444/grid/register -Dphantomjs.binary.path=$PHANTOMJS_BIN_PATH"
...
do_start()
{
    ...
    echo "COMMAND: java -jar $SELENIUM_APP $SELENIUM_OPT >$LOG 2>$ERROR_LOG &"
    java -jar $SELENIUM_APP $SELENIUM_OPT >$LOG 2>$ERROR_LOG &
    error=$?
